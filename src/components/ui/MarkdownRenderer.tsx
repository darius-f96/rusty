  ...
  <div className={`markdown-content text-left ${className}`}>
    <ReactMarkdown
      ...
      components={{
        ...
        a: ({ children, href }) => (
          <a href={href} target="_blank" rel="noopener noreferrer" className="text-[var(--color-primary)] hover:opacity-80 underline font-semibold transition-colors">
            {children}
          </a>
        ),
        ...
        blockquote: ({ children }) => <blockquote className="border-l-4 border-[var(--color-primary)] pl-3 py-1 my-2 bg-[var(--color-interaction-hover)] rounded-r text-[var(--color-fg-muted)] italic">{children}</blockquote>,
        ...
        code: ({ children, className: codeClassName, ...props }) => {
          ...
          return isBlock ? (
            ...
            <code className="px-1.5 py-0.5 rounded bg-[var(--color-log-surface)] border border-[var(--color-border-default)] text-[var(--syntax-keyword)] font-mono text-[11px]" {...props}>{children}</code>
          );
        },
        pre: ({ children }) => <pre className="my-3 whitespace-pre-wrap break-words rounded-lg border border-[var(--color-border-default)] bg-[var(--color-log-background)] text-[var(--color-log-foreground)] p-3">{children}</pre>,
      }}
    >
      {content}
    </ReactMarkdown>
