...
export function createRunCommandTool(options: RunCommandToolOptions) {
  return {
    name: "run_command",
    description: "Run one non-interactive executable in the physical workspace after explicit user approval. Use separate program and args fields; do not wrap commands in sh or bash.",
    inputSchema: {
      type: "object",
      properties: {
